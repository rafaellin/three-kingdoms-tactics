...
### 大地图
- [x] 回合制轮换（E 键 / 右侧面板「结束回合」按钮结束回合；天数+1 自动每日结算，见"资源与经济"）
...
- [x] 回合轮转触发每日结算（天数+1、圈回第一玩家时结算一次；`applyDailyIncome` 按玩家循环结算）
...
### P3 — 内容与打磨
- [ ] 主菜单（开始/载入/设置）—— 已有「探索测试」「开始战役」「战斗测试」三按钮 + Loading 页 + 淡入动画；缺少「载入存档」「设置」入口
- [ ] 小地图
- [ ] 关卡/剧本系统
- [ ] 多种胜利条件
- [ ] 动画效果
