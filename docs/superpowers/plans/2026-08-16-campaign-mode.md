...
**东岭关地图布局（hex 坐标规划，半径 3 = 37 格，窄路真·唯一通道）：**
```
         . . . .
        . 兵 兵 . .
       . 周 关 孙 # .
      . . . 城 . # .
       . # # 孔 # #
        . # # # #
         . . . .
```
> 图例：`.`=plain 可通行；`#`=mountain 不可通行（窄路封死）；`城`=东岭小城 `(0,0)`（出生地）；`孔`=孔秀守将 `(0,1)`（唯一通道，存活时不可通行）；`关/周/孙`=关羽/周仓/孙乾 三武将出生点；`兵`=中立杂兵。
- 初始城：`{ q: 0, r: 0 }`
- 三武将出生：`(0,-1)` 关羽 / `(-1,-1)` 周仓 / `(1,-1)` 孙乾
- 窄路关卡（真·唯一通道）：从 `(0,0)` 向南——`(0,1)` 为孔秀格（plain，唯一通道）；两侧 `(-1,1)`/`(1,1)`、东侧翼 `(2,-1)`/`(2,0)`/`(2,1)`、西侧翼 `(-2,1)`/`(-2,2)`、关后 `(0,2)`/`(-1,2)`/`(1,2)` 全部 `mountain` 封死——无法绕关，只能正面击破孔秀后通过（flood-fill 从 `(0,0)` 跳过 `(0,1)` 到不了关后）
